...
func (t *TransactionRepositoryDb)  SaveTransaction(transaction domain.Transaction, creditCard domain.CreditCard) error {
	stmt, err := t.db.Prepare(query: `insert into transactions(id, creadit_card_id, amount, status, description, store, created_at)
...
func (t *TransactionRepositoryDb)  GetCreditCard(creditCard domain.CreditCard) (domain.CreditCard, error) {
	...
	stmt, err := t.db.Prepare(query: "select balance, balance_limit from credit_cards where number=$1")
	if err != nil {
		return c, err
	}
